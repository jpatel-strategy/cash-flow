...
## 2026-09-16 — Milestone 6: Power BI implementation-ready handoff package

**No native .pbix was created, because none could be.** This
environment has no Power BI Desktop application and no Power BI
REST/XMLA API access, so there is no way to author or programmatically
verify a real `.pbix` here. Per General Rule 17 and the explicit
Milestone 6 instruction to "never claim that a PBIX was created if it
was not," built `deliverables/powerbi_handoff/` as a complete,
implementation-ready handoff package instead, via
`scripts/build_powerbi_handoff.py`.

**Star schema**: 5 dimension tables (`dim_scenario`, `dim_fiscal_year`,
`dim_metric` [91 metric keys, heuristically labeled/categorized —
presentation only, no fabricated values], `dim_filing`,
`dim_date_quarterly`) and 9 fact tables, all exported live from
`data/curated/target_cash.db` — the same production database every
other deliverable reads. `fact_filing_vintage_comparison` (20 rows) is
the one computed (not raw-table) export: pivoting
`fact_annual_historical` to compare `as_originally_filed` against
`latest_restated` for revenue/cost_of_sales/gross_profit/
operating_expenses, reproducing the same FY2022/FY2023 COGS-vs-SG&A
reclassification finding already on record from Milestone 2 (zero
revenue impact, $77M/$92M reclassified within cost lines).

**Deliverables**: `README.md` (package scope and honest PBIX-not-created
disclaimer), `data_dictionary.md`, `relationship_map.md` (every
fact-to-dimension relationship with cardinality/cross-filter direction),
`dax_measures.md` (the 13 required core measures — revenue growth,
gross margin, operating margin, CFO, FCF, FCF margin, post-dividend
capacity, minimum-cash requirement, debt reserve, deployable capacity,
net debt, scenario variance, validation status — plus supporting
measures, all sourced from fact-table columns, none hardcoded),
`theme.json` (a valid Power BI report-theme JSON matching the Excel
workbook's navy/gold palette), `refresh_instructions.md`,
`validation_totals.md` (independent totals a developer can check a
built report against, including the FY2025 CFO $6,562M / CapEx $3,727M
/ CFI $(3,649)M / FCF $2,835M reference values, re-verified not to
collapse CapEx into CFI), and 8 page specs + 8 schematic SVG wireframe
mockups (`pages/*.md` + `wireframes/*.svg`) covering: Executive
Overview, Historical Financial Trends, Filing-Vintage Comparison,
Quarterly Cash Proof, Scenario Forecast Explorer, Cash-Flow Bridge &
Investment Capacity, Capital Allocation Waterfall, and DCF Valuation &
Sensitivities.

**Wireframes are labeled, honestly, as schematic mockups** (plain SVG
box-and-label diagrams generated by the build script, each carrying an
explicit "not an actual Power BI screenshot" caption) — not real
product screenshots, since no Power BI Desktop was available to capture
real ones.

**Verification** (`scripts/verify_powerbi_handoff.py`, 57 checks): every
exported CSV's row count matches the live database exactly; zero orphan
foreign keys anywhere in the star schema (every scenario_id, fiscal_year,
metric, accession_number, and as_of_date referenced by a fact table
exists in its dimension table); the documented validation totals (229/229
forecast checks PASS, 28/28 valuation checks PASS, the 4 FY2025 reference
figures) reproduce exactly from the exported data; every required file
and page spec is present; all 8 wireframe SVGs are well-formed XML; and
explicitly, no `.pbix` file exists anywhere in the package. All checks
pass. Full pytest suite unaffected: 401 passed.

**Known limitation**: this package has never been opened in actual
Power BI Desktop, so a residual risk of a real-product-specific issue
