...
`data/curated/target_cash.db` (SQLite), 28 tables, 6,658 rows as of the
...
| `capacity_horizon_results` | one row per (scenario, version) | Five-year cumulative reconciliation: `gross_horizon_funding_before_reserve_adjustments` (the legacy column, computed BEFORE any reserve deduction — **not** accessible capacity), cumulative self-funded/debt-funded generation, cumulative discretionary deployment, terminal remaining headroom, ending reserve-movement, a terminal forward debt-repayment reserve term, and (final independent-audit closeout, current) `net_horizon_deployable_capacity` — the corrected NET headline figure, equal to gross funding minus both reserve terms, which reconciles exactly to cumulative discretionary deployment + terminal remaining headroom. Expected `net_horizon_deployable_capacity`: Base $9,175.0M, Upside $7,420.7M, Downside $6,387.5M — never publish the gross figures ($9,303.9M / $8,527.1M / $6,092.3M) as "accessible capacity." |
| `capacity_taxonomy_lineage` | one row per field-level dependency | Formula-and-input lineage for every new derived capacity-taxonomy field. As of the final independent-audit closeout, `forward_debt_repayment_reserve` for FY2026-FY2029 is recorded with `dependency_timing='next_year'` and explicit next-fiscal-year `debt_proceeds`/`debt_repayments` fact-ID citations (it is NOT a same-year input); FY2030's row is recorded with `dependency_timing='terminal_proxy'` and a `proxy_note` disclaiming any actual FY2031 obligation. |
| `capacity_validation_results` | one row per check per scenario/year | 213 automated validation results per version proving the 19 named capacity-taxonomy checks (conservation, non-negativity, cross-checks against `ending_excess_liquidity`, the horizon reconciliation identity, and — as of the final independent-audit closeout — the gross/net horizon-capacity dual-identity reconciliation). Per-year lineage integrity checks require an exact match on `scenario_id + fiscal_year + version`; horizon lineage integrity is checked separately, also on an exact version match. |
