...
def test_compute_capacity_preflight_needs_no_database():
    ...
    assert summary["capacity_taxonomy_lineage"] == 3 * (14 * 5 + 7)  # per-year + horizon fields
...
def test_persist_writes_expected_counts():
    ...
    assert conn.execute("SELECT COUNT(*) FROM capacity_taxonomy_lineage").fetchone()[0] == 3 * (14 * 5 + 7)
    assert conn.execute("SELECT COUNT(*) FROM capacity_validation_results").fetchone()[0] == 147
